/*
 * 简单的echo server，默认监听本机54321端口
 * 接收client端连接，然后把客户端发送的数据加上时间戳后发送回去
 * 客户端使用telent
 */
...
{
    /*
     * sets up a socket listening on local port
     *
...
void echo_request(int conn_fd)
{
    ...
    while( (n = recv(conn_fd, recvBuff, sizeof(recvBuff), 0)) > 0 )
    {
        ...
        recvBuff[n] = '\0';
        printf("recvived: %s", recvBuff);
        strcat(sendBuff, recvBuff);
        send(conn_fd, sendBuff, sizeof(sendBuff), 0);
    }
